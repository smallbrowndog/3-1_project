...
70퍼센트씩 추출해 학습하여 결과를 예측했을때는 아래와 같은 결과가 나왔고  
<img src="https://github.com/smallbrowndog/3-1_project/assets/136410944/618a2e71-0919-433d-bf6f-eeac4fafeaae">  
각 2000개씩, 총 4000개를 추출해 학습하여 결과를 예측했을때는 아래와 같은 결과과 나왔다.  
<img src="https://github.com/smallbrowndog/3-1_project/assets/136410944/10a099bf-d777-4b8a-9022-2b7b5fa305d7">

2000개씩 추출한 데이터보다 70퍼센트씩를 추출해 학습후 결과를 예측했을때가 더욱 정확도가 높은 모습이 보였다.

## 5. 느낀점 및 배운점

아마존 패션 리뷰 데이터를 활용한 감성 분석 프로젝트를 진행하면서 다음과 같은 점들을 깨달을 수 있었다.  

첫째, 데이터의 전처리의 중요성을 깨닫게 되었다. 내가 이용했던 패션 상품 리뷰 데이터는 내가 원하지 않았던 패션 아이템이 섞여 있었기때문에 정확한 결과값을 위해 같은 카테고리의 의류들만 추출하는것이 필요했다. 만약 이를 진행하지 않고 모두 섞어서 돌렸다면 패션 아이템마다 사용하는 명칭, 중요하게 생각하는 면들이 달랐기때문에 정확하지 않은 결과가 나왔을것이다.

둘째, 데이터 불균형 문제에 대한 대처 방안을 고민해볼 수 있었다. 긍정 리뷰와 부정 리뷰의 비율이 균형을 이루지 않아, 이를 해결하기 위한 방법을 고민해보고 실천해 볼 수 있었다. 예를 들어 각 똑같은 개수를 추출하는것과 각각의 갯수중 같은 비율로 추출하는것의 방법을 적용해보며 어떤 것이 더 효과적인지 알 수 있었다.  

셋째, 모델 선택의 중요성을 깨달았다. 앞서 말했던것처럼 2000개씩 추출한 모델, 70%씩 추출한 모델의 차이를 직접 확인하며 적절한 모델을 찾는것이 중요하다는 것을 알 수 있었다.  

마지막으로, 이러한 프로젝트를 통해 자연어 처리와 감성 분석에 대한 이해도를 높일 수 있었다. 실제 데이터를 다루면서 이론적인 내용을 실습해볼 수 있었고, 내가 혼자 해본 감성분석 모델의 정확도가 높게 나와서 의외였다.  

아쉬운 점으로는 내가 원하던 패션 아이템, 패션 리뷰가 아니었기때문에 도중에 흥미가 떨어졌던것이 아쉬웠다. 나중에는 내가 원하는 데이터를 가지고 실습해본다면 더 좋을 것 같았다.
