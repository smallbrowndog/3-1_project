...
model_path = 'mobilebert_model.pt'
model = MobileBertForSequenceClassification.from_pretrained(model_path)
model.eval()

PA_fin_df = pd.read_csv('project_tot_text.csv.csv')
data_X = list(PA_fin_df['tot_text'].values)
labels = PA_fin_df['pos_neg'].values
...
test_lables = torch.tensor(labels)
test_masks = torch.tensor(attention_mask)
test_dataset = TensorDataset(test_inputs, test_masks, test_lables)
